...
setupLazyAdminReads(){
  ...
  const drainReadQueue=()=>{
    ...
    if(!job || !job.element)return drainReadQueue();
    readBusy=true;
    Promise.resolve()
      .then(()=>this[job.methodName]())
      .catch(e=>{
  ...
  const loadJob=(element,methodName)=>{
    ...
    readQueue.push({element,methodName});
    drainReadQueue();
  };
...
bindProviderConfig(){document.getElementById("providerConfigStatusBtn")?.addEventListener("click",()=>this.loadProviderConfigStatus());document.getElementById("providerConfigTestBtn")?.addEventListener("click",()=>this.runProviderConfigTest())},bindPaymentRouting(){document.getElementById("paymentRoutingStatusBtn")?.addEventListener("click",()=>this.loadPaymentRoutingStatus());document.getElementById("paymentRoutingTestBtn")?.addEventListener("click",()=>this.runPaymentRoutingTest())},bindPaymentReconciliation(){document.getElementById("paymentReconciliationStatusBtn")?.addEventListener("click",()=>this.loadPaymentReconciliationStatus());document.getElementById("paymentReconciliationTestBtn")?.addEventListener("click",()=>this.runPaymentReconciliationTest())},bindMerchantContract(){document.getElementById("merchantContractStatusBtn")?.addEventListener("click",()=>this.loadMerchantContractStatus());document.getElementById("merchantUpgradeTestBtn")?.addEventListener("click",()=>this.runMerchantUpgradeTest())},bindUniversalPaymentApi(){document.getElementById("universalPaymentApiTestBtn")?.addEventListener("click",()=>this.runUniversalPaymentApiTest())},bindMerchantRegistry(){document.getElementById("merchantRegistryStatusBtn")?.addEventListener("click",()=>this.loadMerchantRegistryStatus());document.getElementById("merchantRegistryTestBtn")?.addEventListener("click",()=>this.runMerchantRegistryTest())},bindPaymentIntentLifecycle(){document.getElementById("paymentIntentLifecycleTestBtn")?.addEventListener("click",()=>this.runPaymentIntentLifecycleTest())},bindConcurrencyTest(){document.getElementById("concurrencyTestBtn")?.addEventListener("click",()=>this.runConcurrencyTest())},bindMismatchConcurrencyTest(){document.getElementById("mismatchConcurrencyTestBtn")?.addEventListener("click",()=>this.runMismatchConcurrencyTest())},bindRecoveryReplayTest(){document.getElementById("recoveryReplayTestBtn")?.addEventListener("click",()=>this.runRecoveryReplayTest())},
...
try{
const idToken=await currentUser.getIdToken(true);
const response=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"provider_routing_status",idToken})});
const result=await response.json();
if(!result.success)throw new Error(result.error||"Gagal membaca payment channel routing.");
...
}catch(e){console.error(e);m.textContent="Payment channel routing gagal: "+e.message}
},
...
try{
const idToken=await currentUser.getIdToken(true);
const response=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_provider_routing_test",idToken})});
const result=await response.json();
if(!result.success)throw new Error(result.error||"Payment Channel Routing Test gagal.");
const detail=(result.checks||[]).map(x=>`${x.pass?"PASS":"FAIL"}: ${x.name}`).join(" · ");
...
try{
const idToken=await currentUser.getIdToken(true);
const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_missing_receipt_test",idToken})});
...
try{
const idToken=await currentUser.getIdToken(true);
const prepResponse=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_concurrency_prepare",idToken})});
const prep=await prepResponse.json();
if(!prep.success)throw new Error(prep.error||"Gagal menyiapkan concurrency test.");
const webhookCount=Number(prep.concurrencyWebhookCount||20);
const reconCount=Number(prep.concurrencyReconciliationCount||20);
const webhookBody={action:"sandbox_webhook",idToken,paymentIntentId:prep.paymentIntentId,provider:prep.provider,providerEventId:prep.providerEventId,eventType:prep.eventType,providerReference:prep.providerReference,payloadHash:"CONCURRENCY-HASH-2351",currency:prep.currency,amount:prep.amount,targetStatus:prep.targetStatus,signatureValid:true};
m.textContent=`Menjalankan ${webhookCount} duplicate webhook bersamaan...`;
const webhookResults=await Promise.all(Array.from({length:webhookCount},()=>fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify(webhookBody)}).then(r=>r.json())));
...
const reconBody={action:"reconciliation_query",idToken,paymentIntentId:prep.paymentIntentId};
const reconResults=await Promise.all(Array.from({length:reconCount},()=>fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify(reconBody)}).then(r=>r.json())));
const reconOk=reconResults.filter(x=>x.success).length;
const verifyResponse=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_concurrency_verify",idToken,paymentIntentId:prep.paymentIntentId,providerEventId:prep.providerEventId,webhookCount,reconciliationCount:reconCount})});
...
}catch(e){console.error(e);m.textContent="Concurrency Test gagal: "+e.message}
},
...
try{
const idToken=await currentUser.getIdToken(true);
const prepResponse=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_mismatch_concurrency_prepare",idToken})});
const prep=await prepResponse.json();
...
const results=await Promise.all(bodies.map(body=>fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify(body)}).then(async r=>{try{return await r.json()}catch(e){return {success:false,error:"Invalid JSON response"}}})));
const rejected=results.filter(x=>x.rejected||x.success===false).length;
const verifyResponse=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_mismatch_concurrency_verify",idToken,paymentIntentId:prep.paymentIntentId,providerEventId:prep.providerEventId,requestCount:n})});
const result=await verifyResponse.json();
const detail=(result.checks||[]).map(c=>`${c.pass?"PASS":"FAIL"}: ${c.name}${c.detail?` (${c.detail})`:""}`).join(" · ");
...
}catch(e){console.error(e);m.textContent="Mismatch Concurrency Test gagal: "+e.message}
},
...
try{
const idToken=await currentUser.getIdToken(true);
const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_recovery_replay_test",idToken})});
const x=await r.json();
const detail=(x.checks||[]).map(c=>`${c.pass?"PASS":"FAIL"}: ${c.name}${c.detail?` (${c.detail})`:""}`).join(" · ");
m.textContent=`Recovery & Replay Test ${x.overall||"FAIL"}: PASS ${x.passCount||0} · FAIL ${x.failCount||0}. ${x.message||x.error||""} ${detail}`;
}catch(e){console.error(e);m.textContent="Recovery & Replay Test gagal: "+e.message}
},
...
if(!auth||!currentUser){m.textContent="Login admin terlebih dahulu.";return}
if(!id){m.textContent="Masukkan Payment Intent ID pada panel Payment Reconciliation.";return}
m.textContent="Menjalankan Reconciliation Integrity 23.5.1...";
try{
const idToken=await currentUser.getIdToken(true);
const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_reconciliation_integrity_test",idToken,paymentIntentId:id})});
const x=await r.json();
const detail=(x.checks||[]).map(c=>`${c.pass?"PASS":"FAIL"}: ${c.name}${c.detail?` (${c.detail})`:""}`).join(" · ");
m.textContent=`Reconciliation Integrity ${x.overall||"FAIL"}: PASS ${x.passCount||0} · FAIL ${x.failCount||0}. ${x.message||x.error||""} ${detail}`;
}catch(e){console.error(e);m.textContent="Reconciliation Integrity gagal: "+e.message}
},
...
if(!auth||!currentUser){m.textContent="Login admin terlebih dahulu.";return}
if(!id){m.textContent="Masukkan Payment Intent ID.";return}
try{
const idToken=await currentUser.getIdToken(true);
const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"payment_reconciliation_status",idToken,paymentIntentId:id})});
const x=await r.json(); if(!x.success)throw new Error(x.error||"Gagal membaca reconciliation.");
...
}catch(e){console.error(e);m.textContent="Reconciliation status gagal: "+e.message}
},
async runPaymentReconciliationTest(){
const m=document.getElementById("paymentReconciliationTestMessage");
const id=document.getElementById("paymentReconciliationIntentId")?.value.trim();
if(!auth||!currentUser){m.textContent="Login admin terlebih dahulu.";return}
if(!id){m.textContent="Masukkan Payment Intent ID SUCCESS yang sudah memiliki PAID transaction/payment.";return}
m.textContent="Menjalankan Payment Receipt & Reconciliation Test...";
try{
const idToken=await currentUser.getIdToken(true);
const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_payment_reconciliation_test",idToken,paymentIntentId:id})});
const x=await r.json();
const detail=(x.checks||[]).map(c=>`${c.pass?"PASS":"FAIL"}: ${c.name}${c.detail?` (${c.detail})`:""}`).join(" · ");
...
}catch(e){console.error(e);m.textContent="Payment Receipt & Reconciliation Test gagal: "+e.message}
},
async loadMerchantContractStatus(){
const m=document.getElementById("merchantContractStatusMessage");if(!auth||!currentUser){m.textContent="Login admin terlebih dahulu.";return}
try{const idToken=await currentUser.getIdToken(true);const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"merchant_contract_status",idToken})});const x=await r.json();if(!x.success)throw new Error(x.error||"Gagal membaca merchant contract.");m.textContent=`Contract ${x.contractVersion} · ${x.merchantContract} · Purposes ${(x.paymentPurposes||[]).join(", ")} · Channels ${(x.channels||[]).map(c=>c.channel).join(", ")} · Credential exposed: ${x.credentialValuesExposed?"YES":"NO"}.`;}catch(e){console.error(e);m.textContent="Merchant contract gagal: "+e.message}
},
async runMerchantUpgradeTest(){
const m=document.getElementById("merchantUpgradeTestMessage");if(!auth||!currentUser){m.textContent="Login admin terlebih dahulu.";return}m.textContent="Menjalankan Universal Merchant & Upgrade Test...";
try{const idToken=await currentUser.getIdToken(true);const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_merchant_upgrade_test",idToken,merchantId:"DEMO-TICKETING",applicationId:"DEMO-TICKETING-APP",sourceReference:"TICKET-ECONOMY-001",targetReference:"TICKET-VIP-001",previousAmount:100000,targetAmount:250000})});const x=await r.json();const detail=(x.checks||[]).map(c=>`${c.pass?"PASS":"FAIL"}: ${c.name}${c.detail?` (${c.detail})`:""}`).join(" · ");m.textContent=`Universal Merchant & Upgrade ${x.overall||"FAIL"}: PASS ${x.passCount||0} · FAIL ${x.failCount||0}. ${x.message||x.error||""} ${detail}`;}catch(e){console.error(e);m.textContent="Universal Merchant & Upgrade Test gagal: "+e.message}
},
async loadMerchantRegistryStatus(){
const m=document.getElementById("merchantRegistryStatusMessage");
if(!auth||!currentUser){m.textContent="Login admin terlebih dahulu.";return}
try{const idToken=await currentUser.getIdToken(true);const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"merchant_registry_status",idToken})});const x=await r.json();if(!x.success)throw new Error(x.error||"Gagal membaca registry.");m.textContent=`Registry ${x.registryVersion} · Merchant ${x.merchantCount} · Raw secret returned: ${x.rawSecretsReturned?"YES":"NO"} · Credential exposed: ${x.credentialValuesExposed?"YES":"NO"}.`;}catch(e){console.error(e);m.textContent="Merchant Registry gagal: "+e.message}
},
...
try{const idToken=await currentUser.getIdToken(true);const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"sandbox_merchant_registry_test",idToken})});const x=await r.json();const detail=(x.checks||[]).map(c=>`${c.pass?"PASS":"FAIL"}: ${c.name}${c.detail?` (${c.detail})`:""}`).join(" · ");m.textContent=`Merchant Registry & API Authentication ${x.overall||"FAIL"}: PASS ${x.passCount||0} · FAIL ${x.failCount||0}. ${x.message||x.error||""} ${detail}`;}catch(e){console.error(e);m.textContent="Merchant Registry Test gagal: "+e.message}
},
...
try{const idToken=await currentUser.getIdToken(true);const r=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"universal_payment_api_test",idToken})});const x=await r.json();const detail=(x.checks||[]).map(c=>`${c.pass?"PASS":"FAIL"}: ${c.name}${c.detail?` (${c.detail})`:""}`).join(" · ");m.textContent=`Universal Merchant Payment API ${x.overall||"FAIL"}: PASS ${x.passCount||0} · FAIL ${x.failCount||0}. ${x.message||x.error||""} ${detail}`;}catch(e){console.error(e);m.textContent="Universal Merchant Payment API Test gagal: "+e.message}
},
...
if(this.checkoutBusy){m.textContent="Checkout sedang diproses. Tunggu request sebelumnya selesai.";return}
const button=document.querySelector("#checkoutForm button[type='submit']");
this.checkoutBusy=true;if(button)button.disabled=true;
m.textContent="Memvalidasi Checkout melalui trusted backend...";
try{
const idToken=await currentUser.getIdToken(true);
const response=await fetch(config.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action:"create_checkout_session",idToken,paymentIntentId:intentId,paymentMethodId:pmId,eventId})});
const result=await response.json();
if(!result.success)throw new Error(result.error||"Gagal membuat checkout.");
